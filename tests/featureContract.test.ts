...
export function runFeatureContractTests(): { passed: number; failed: number; errors: string[] } {
  ...
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ featureId: '' })); } catch { threw = true; }
    assert(threw, '2. Empty featureId is rejected');
  }

  // 3. name is required.
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ name: '  ' })); } catch { threw = true; }
    assert(threw, '3. Blank name is rejected');
  }

  // 4. Writing an entity type never declared as read is rejected.
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ writeEntityTypes: ['WATER_TANK'], readEntityTypes: [] })); } catch { threw = true; }
    assert(threw, '4. Writing an entity type not in readEntityTypes is rejected');
  }
  ...
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ liveImpact: 'ISSUES_CONTROL_COMMANDS', writeEntityTypes: [] })); } catch { threw = true; }
    assert(threw, '6. liveImpact ISSUES_CONTROL_COMMANDS with empty writeEntityTypes is rejected');
  }

  // 7. createsObservations with liveImpact NONE is rejected.
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ createsObservations: true, liveImpact: 'NONE' })); } catch { threw = true; }
    assert(threw, '7. createsObservations with liveImpact NONE is rejected');
  }
  ...
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ createsExperiments: true, simulationImpact: 'NONE' })); } catch { threw = true; }
    assert(threw, '9. createsExperiments with simulationImpact NONE is rejected');
  }

  // 10. evidenceImpact PRODUCES_EVIDENCE with neither observations nor experiments is rejected.
  {
    let threw = false;
    try { validateFeatureDefinition(baseFeature({ evidenceImpact: 'PRODUCES_EVIDENCE' })); } catch { threw = true; }
    assert(threw, '10. evidenceImpact PRODUCES_EVIDENCE with no observation/experiment source is rejected');
  }
