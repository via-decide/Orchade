...
export class ReplayRecorder {
  ...
  record(frame: ReplayFrame, state?: unknown): void {
    this.frames.push(frame);
    if (state !== undefined && frame.tick % this.snapshotIntervalTicks === 0 && frame.checksum) {
      this.snapshots.push({ tick: frame.tick, state, checksum: frame.checksum });
    }
  }
  ...
  nearestCheckpoint(tick: number): ReplaySnapshot | undefined {
